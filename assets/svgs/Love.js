...
import { COLOR_WHITE, COLOR_PINK } from '../../app/styles';
/* SVGR has dropped some elements not supported by react-native-svg: style */

const SvgLove = props => {
  const color = props.active ? COLOR_PINK : 'none';
  const colorSeccond = props.active ? COLOR_WHITE : COLOR_PINK;
  return (
    <Svg height={48} width={48} viewBox="5 0 48 48" {...props}>
      <G fill="none">
        <Path
          fill={color}
          stroke={COLOR_PINK}
